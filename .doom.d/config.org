...
(setq org-directory "~/roam_notes/notes/")
#+end_src
** Org noter
#+begin_src elisp
(setq org-noter-notes-search-path '("~/roam_notes/notes/")) ;;basic org config
#+end_src
** Org Capture
...
(after! (:or org-capture org-agenda org)
    (setq org-capture-templates
      (doct '(("UNI" :keys "u"
               :file "~/roam_notes/notes/agenda/2021.2/2021.2.org"
               :prepend t
               :template ("* %{todo-state} %?"
                          "%i %a")
              ...
              ("Appointments" :keys "p"
               ...
               :template ("* %^{Description} %?"
                          "%i"))
              ("Projects" :keys "P"
               :file "~/roamnotes/notes/agenda/2021.2/2021.2.org"
               :append t
               :headline "Projects"
               :type entry
               :template ("* %^{Description} %?"
...
(use-package deft
  ...
  (deft-directory "~/roam_notes/notes/org/")) ;;notetaking-deft
#+end_src
      ...
      '(("o" "Overview of the day"
         (
          ...
          (org-ql-block '(and
            ...
            (org-ql-block '(and
                          (deadline :from "-7" :to "-1")
                          (path "2021.2/")
                          )
                        ((org-ql-block-header "Overdue tasks for the week")))
          (agenda "" ((org-agenda-span 'day)
                      (org-super-agenda-groups
                       '((:name "Today Tasks"
                          :time-grid t
                          :date today
                          :todo ("TODAY" "STRT" "HOLD")
                          :scheduled today
                          :order 1
                          :discard (:anything t))))))
          )
...
(defun my-open-calendar ()
  ...
  (cfw:open-calendar-buffer
   ...
   (list
    ...
    (cfw:org-create-file-source "cal" "~/roam_notes/notes/agenda/2021.2/2021.2.org" "Blue")  ; other org source
    )))
#+end_src
** Roam
#+begin_src elisp
(setq org-roam-directory "~/roam_notes/notes/org/")
